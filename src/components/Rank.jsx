...
const DEFAULT_WEIGHTS = { goal: 2, assist: 1, voto: 3, vittorie: 1.5, portiere: 2 };
...
export function Rank({ players, matches }) {
  ...
  const stats = players.map(p => {
    ...
    for (const m of completed) {
      ...
      const mv = s.voto || 0;
      const isGoalie = (in1 && p.id === m.goalie1) || (in2 && p.id === m.goalie2);
      ...
      const matchPts =
        ...
        ma * weights.assist +
        mv * weights.voto +
        (won ? weights.vittorie : 0) +
        (isGoalie ? weights.portiere : 0);
  ...
  return (
    <div style={S.page}>
      <h2 style={S.pageTitle}>🏆 Classifica Generale</h2>

      {/* Legenda formula con pesi reali */}
      <div style={{ background:"#0a1a0a",border:"1px solid #1a3a1a",borderRadius:10,padding:"8px 12px",marginBottom:14,fontSize:11,color:"#4a7a3a",lineHeight:1.8 }}>
        ...
        ⚽ Goal ×{weights.goal} &nbsp;·&nbsp; 🅰️ Assist ×{weights.assist} &nbsp;·&nbsp; ⭐ Voto ×{weights.voto} &nbsp;·&nbsp; 🏆 Vittoria +{weights.vittorie} &nbsp;·&nbsp; 🧤 Portiere +{weights.portiere}
      </div>
