  ...
  return (
    <div style={{ background:"#06140a",border:"1px solid #1a3a1a",borderRadius:12,padding:12,marginTop:10 }}>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10 }}>
        <div>
          {/* Aggiunto il controllo per il portiere direttamente nell'intestazione del capitano */}
          <div style={{ color:"#4fc3f7",fontWeight:800,fontSize:12,marginBottom:4 }}>
            🔵 {pn(lm.cap1)} <span style={{fontSize:10,color:"#7a9e5a"}}>(cap)</span>
            {lm.cap1 === lm.goalie1 && " 🧤"}
          </div>
          {/* Modificata la logica delle icone per permetterne più di una contemporaneamente */}
          {(lm.team1 || []).map(id => (
            <div key={id} style={{ fontSize:12,color:"#9ab87a" }}>
              {pn(id)}
              {id === lm.goalie1 && " 🧤"}
              {id === lm.cap1 && " ©"}
            </div>
        ...
        <div>
          {/* Aggiunto il controllo per il portiere direttamente nell'intestazione del capitano */}
          <div style={{ color:"#ff8a65",fontWeight:800,fontSize:12,marginBottom:4 }}>
            🟠 {pn(lm.cap2)} <span style={{fontSize:10,color:"#7a9e5a"}}>(cap)</span>
            {lm.cap2 === lm.goalie2 && " 🧤"}
          </div>
          {/* Modificata la logica delle icone per permetterne più di una contemporaneamente */}
          {(lm.team2 || []).map(id => (
