...
export function Games({ players, matches, getMatches, user, saveMatches, setTab }) {
  ...
  const create = async () => {
    ...
    // team1 e team2: se il capitano è anche portiere non va duplicato
    const team1 = form.cap1 === form.goalie1 ? [form.cap1] : [form.cap1, form.goalie1];
    const team2 = form.cap2 === form.goalie2 ? [form.cap2] : [form.cap2, form.goalie2];
    const m = {
        ...
        <div style={{ background:"#0f220f",border:"1px solid #2d5a1e",borderRadius:14,padding:16,marginBottom:16 }}>
          ...
          <Lbl>Portiere 🧤🔵
            <select style={S.inp} value={form.goalie1} onChange={e => setForm(f=>({...f,goalie1:e.target.value}))}>
              <option value="">— scegli —</option>
              {players.filter(p => p.id !== form.goalie2 && p.id !== form.cap2).map(p =>
                <option key={p.id} value={p.id}>{p.name}{p.id === form.cap1 ? " ©" : ""}</option>
              )}
            </select>
          </Lbl>
          <Lbl>Capitano 🟠
          ...
          <Lbl>Portiere 🧤🟠
            <select style={S.inp} value={form.goalie2} onChange={e => setForm(f=>({...f,goalie2:e.target.value}))}>
              <option value="">— scegli —</option>
              {players.filter(p => p.id !== form.goalie1 && p.id !== form.cap1).map(p =>
                <option key={p.id} value={p.id}>{p.name}{p.id === form.cap2 ? " ©" : ""}</option>
              )}
            </select>
          </Lbl>
